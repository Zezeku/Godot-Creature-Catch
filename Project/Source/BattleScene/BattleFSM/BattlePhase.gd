...
func enter(fsm:StateMachine):
	...
	for creature in attackOrder.size():
		...
		if attackOrder[creature].get_parent() != null: #what is this doing?
...
func decide_next_state():
	...
	#should check party child count instead
	#fainted implemented, can do this now
	if (Player.getPartySize() == 0) or (Enemy.getPartySize() == 0):
		change_state("BattleEnd");
	else:
		#need to check for a swap here, for both dies
